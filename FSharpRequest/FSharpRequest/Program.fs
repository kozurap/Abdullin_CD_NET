...
let calcualte a op b url = Async.RunSynchronously (asyncMaybe{
        let! a = async{
...
let ProxyDependency calculate a op b url =
    calculate a op b url
[<EntryPoint>]
let main args =
    ...
    let expression = ProxyDependency calcualte a op b url
    write expression
